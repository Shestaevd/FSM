...
namespace Fsm.Script
{
    public class StateManager<T>
    {
            ...
            switch (rule)
            {
                case 1:
                    foreach (AbstractState<T> state in States)
                        if (current.WhiteSet.Contains(state) && state.EnterCondition(Entity) && current.priority < state.priority && !current.Equals(state))
                        {
                            current = state;
                            return true;
                        }
                ...
                case -1:
                    foreach (AbstractState<T> state in States)
                        if (!current.BlackSet.Contains(state) && state.EnterCondition(Entity) && current.priority < state.priority && !current.Equals(state))
                        {
                            current = state;
                            return true;
                        }
                ...
                case 0:
                    foreach (AbstractState<T> state in States)
                    {
                        if (state.EnterCondition(Entity) && current.priority < state.priority && !current.Equals(state))
                        {
                            Console.WriteLine("Changing to " + state.Name);
                            current = state;
                            return true;
                        }
                    }
        ...
        public State<T> StateNewInstanse(string name, int priority)
        {
            State<T> newState = new State<T>(name, priority);
            AddState(newState);
            return newState;
        }
        public State<T> StateNewInstanse(string name, int priority, Action<T> stateLogic)
        {
            State<T> newState = new State<T>(name, priority, stateLogic);
            AddState(newState);
            return newState;
        }
        public State<T> StateNewInstanse(string name, int priority, Action<T> stateLogic, Func<T, bool> enterCondition)
        {
            State<T> newState = new State<T>(name, priority, stateLogic, enterCondition);
            AddState(newState);
            return newState;
        }
        public State<T> StateNewInstanse(string name, int priority, Action<T> stateLogic, Action<T> onStateEnter, Action<T> onStateExit)
        {
            State<T> newState = new State<T>(name, priority, stateLogic, onStateEnter, onStateExit);
            AddState(newState);
            return newState;
        }
        public State<T> StateNewInstanse(string name, int priority, Action<T> stateLogic, Action<T> onStateEnter, Action<T> onStateExit, Func<T, bool> enterCondition)
        {
